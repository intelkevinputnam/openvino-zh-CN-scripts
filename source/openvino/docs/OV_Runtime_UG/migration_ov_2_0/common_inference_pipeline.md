# 推理管道{#openvino_2_0_inference_pipeline}
...
如需通过自定义操作加载模型，您需要为这些操作添加扩展。强烈建议您使用 [OpenVINO™ 扩展性 API](../../Extensibility_UG/Intro.md) 编写扩展。但是，您也可以将旧扩展加载到新的 OpenVINO™ 运行时：
...
读取模型采用与[模型创建迁移指南](./graph_construction.md)中的示例相同的结构。

您可以在单次 `ov::Core::compile_model(filename, devicename)` 调用中组合模型读取和编译。

### <a name="perform-preprocessing"></a>2.1 （可选）执行模型预处理

当应用输入数据与模型输入格式不完全匹配时，可能需要进行预处理。请参阅 [API 2.0 中的预处理](./preprocessing.md)了解更多详情。
...
如果需要用 OpenVINO™ 运行时的其他参数配置设备，请参阅[配置设备](./configure_devices.md)。
